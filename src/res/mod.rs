...
pub fn load_bg() -> Pixbuf {
    ...
    let data_stream = MemoryInputStream::from_bytes(&Bytes::from_static(BG));
    Pixbuf::from_stream(&data_stream, None as Option<&Cancellable>).unwrap()
}

pub fn load_logo() -> Pixbuf {
    static DATA: &[u8] = include_bytes!("logo.svg");
    let data_stream = MemoryInputStream::from_bytes(&Bytes::from_static(DATA));
    Pixbuf::from_stream(&data_stream, None as Option<&Cancellable>).unwrap()
}
...
pub fn load_button_pixbufs() -> [Pixbuf; BUTTON_COUNT as usize] {
    ...
    let data_stream = MemoryInputStream::from_bytes(&Bytes::from_static(DATA));
    let master = Pixbuf::from_stream(&data_stream, None as Option<&Cancellable>).unwrap();
